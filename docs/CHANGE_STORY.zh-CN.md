...
默认页回答“我今天应该先看什么”：用一句日常语言说明发生了什么，展示最多 3 个记忆点、与你工作的关系，以及 2–3 条建议先验证的操作。验证建议来自主场景、明确关系和证据边界，但始终标注为建议，不冒充代码事实。

### 理解改法

场景镜头继续负责解释修改怎样工作。

默认首页先给出最多 3 个可记忆的业务结论，再回答一个读者问题，而不是列出所有文件和方法。报告最多提供 5 个场景，每个场景的 OLD 与 NEW 关键对象合计不超过 7 个，关系不超过 6 条。常见场景包括引导与完成度、创建与编排、校验与安全修复、预览与表现、加载/保存/导出和运行时攻击。

首屏只展开主场景。每个场景带有独立 `change_shape`：`ADDED` 和 `REMOVED` 使用单侧全宽能力画布，`MODIFIED` 才使用 OLD/NEW 双栏，避免让空栏占据注意力。技术名称、源码位置、完整 OLD/NEW 三列对照和领域卡片均按需展开。场景还带有独立 `relationship_mode`：只有 `VERIFIED_FLOW` 才把逐条明确关系画成路径卡；`PARALLEL_FACTS` 只能解释为回答同一问题的并列事实，不能绘制方向。

### 核对证据

完整的实现阶段、决策点和技术证据位于第三档。Change Map Lite 继续负责严谨的版本对照，但不再占据默认首屏。读者展开 OLD/NEW 对照后，可以回答“原来怎样、核心变化、现在怎样”。它会先判定变化形态：

| 变化形态 | 首屏表达 |
|---|---|
| `MODIFIED` | 原来怎样 → 核心调整 → 现在怎样 |
| `ADDED` | 原来没有 → 新增核心 → 现在具备 |
| `REMOVED` | 原有结构 → 移除核心 → 当前结果 |
| `TEST_ONLY` | 原有业务范围 → 并列新增检查 → 测试保障结果 |
| `CONFIG_PROTOCOL` | 原数据结构 → 配置/协议变化 → 新数据结构 |
| `PARALLEL` | 原有证据 → 并列变化 → 当前结果 |

每列最多 3 项。存在变化 edge 时，版本变化符号旁会提示详细页还有结构关系证据；三列本身始终只是版本对照，不表示列中对象相互调用。没有 edge 时使用并列标记，并明确说明显示顺序不代表调用顺序。纯新增、纯删除和仅测试修改不会用空 OLD/NEW 框冒充流程。

影响范围和第一项优先风险仍直接显示；完整三列对照、帮助理解比喻与最多 5 张领域卡片收进辅助展开区。方法参数、重复条件、生成代码数量和完整关系不会占据首页。

### 详细思路拆解

详细页按配置、服务端、协议、Unity 编辑器工具、客户端、运行逻辑和测试阶段组织代表对象、关键关系与新增决策点。阶段和决策区默认折叠，每一项保留源码位置或节点/关系证据。完整事实层、原始链路、符号表和全部限制继续保留，但默认折叠。
