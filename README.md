...
1. **快速理解**：Change Map Lite 自动区分修改、纯新增、纯删除、仅测试、配置/协议和并列事实，用一屏展示“原来—核心变化—现在”、影响与首要风险；
...
Change Map Lite 只有在分析结果包含变化关系时才显示流程箭头。没有关系证据时，它把方法或类型显示为并列事实，并明确说明显示顺序不代表调用顺序；纯新增和仅测试修改也不会再显示空的 OLD/NEW 链路框。
